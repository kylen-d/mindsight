...
def test_generated_parser_spec_matches_golden():
    ...
    assert len(spec["actions"]) == len(golden["actions"]) == 166
...
def test_flagspec_table_welds_to_schema_and_tables():
    ...
    assert schema_flags == schema_cli and len(schema_flags) == 80
    assert alias_flags == set(CLI_ALIASES) and len(alias_flags) == 12
    assert excluded_flags == set(EXCLUDED_CLI_FLAGS) and len(excluded_flags) == 31
