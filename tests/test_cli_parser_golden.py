...
which drive generation -- is caught: 160 actions = 117 core (76 schema-cli + 12
CLI_ALIASES + 29 excluded) + 43 plugin.
"""
...
def test_parser_prog_and_action_count():
    ...
    assert len(spec["actions"]) == 160
    assert len(golden["actions"]) == 160
...
def test_census_partition_welds_to_tables():
    ...
    assert len(schema_cli) == 76
    assert len(CLI_ALIASES) == 12
    assert len(EXCLUDED_CLI_FLAGS) == 29
