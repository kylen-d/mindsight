...
which drive generation -- is caught: 165 actions = 122 core (79 schema-cli + 12
CLI_ALIASES + 31 excluded) + 43 plugin.
"""
...
def test_parser_prog_and_action_count():
    ...
    assert len(spec["actions"]) == 165
    assert len(golden["actions"]) == 165
...
def test_census_partition_welds_to_tables():
    ...
    assert len(schema_cli) == 79
    assert len(CLI_ALIASES) == 12
    assert len(EXCLUDED_CLI_FLAGS) == 31
