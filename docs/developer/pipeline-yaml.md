...
On startup, `mindsight/config_compat.py` reads the YAML file, flattens any nested keys into dot-separated paths, and maps each key to its corresponding `argparse` attribute. The resulting values are applied to the argument namespace before the pipeline begins.
