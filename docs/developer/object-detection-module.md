...
The `mindsight/ObjectDetection/` module handles all YOLO-based object detection in MindSight. It consists of four files:
...
**File:** `mindsight/ObjectDetection/detection.py`
...
**File:** `mindsight/ObjectDetection/model_factory.py`
...
- **Device resolution:** delegates to `mindsight/utils/device.py` which follows the priority order CUDA > MPS > CPU when device is `"auto"`.
...
Returns a `RetinaFace` instance used for face-level detection in the gaze tracking pipeline. Adds the `mindsight/GazeTracking/gaze-estimation` directory to `sys.path` if not already present.

## YOLOEVPDetector

**File:** `mindsight/ObjectDetection/object_detection.py`
...
**File:** `mindsight/ObjectDetection/detection_pipeline.py`
