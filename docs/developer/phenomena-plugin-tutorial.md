...
- **Lazy imports for dashboard helpers.** The `_dash()` helper function imports from `mindsight.outputs.dashboard_output` only when called, so the plugin module loads even if the outputs package is not yet initialised.
