...
MindSight groups its many parameters into typed dataclasses defined in `mindsight/pipeline_config.py` and `mindsight/Phenomena/phenomena_config.py`. Each dataclass has a `from_namespace(ns)` classmethod that constructs it from an `argparse.Namespace`.
...
Defined in `mindsight/pipeline_config.py`. Configuration for a single auxiliary video stream mapped to a participant.
