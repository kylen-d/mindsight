...
`FrameContext` is the mutable per-frame data carrier that flows through all pipeline stages. Each stage reads what it needs and writes its results. The class lives in `mindsight/pipeline_config.py` and supports dict-like access (`ctx['key']`, `ctx.get('key', default)`, `'key' in ctx`).
