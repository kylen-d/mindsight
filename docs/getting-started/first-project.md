...
- **Create visual prompts** -- Build a `.vp.json` file with the GUI's VP Builder tab to detect custom objects that standard YOLO classes do not cover.
...
- **Assign participant IDs** -- Map track IDs to participant labels and tag videos with study conditions, either per run (`run.yaml`) or study-wide (`project.yaml` / `participant_ids.csv`). The GUI edits both for you: the **Projects** tab's *Study setup* area writes `project.yaml`, and right-clicking a run in **Analyze Footage** offers *Edit run...*.
