...
CORE_FLAGS: tuple[FlagSpec, ...] = (
    ...
    FlagSpec(flag='--face-model', dest='face_model', schema_path=None, default=None, choices=('mnet025', 'mnet050', 'mnet_v1', 'mnet_v2', 'r18', 'r34'), metavar='NAME', help='RetinaFace backbone variant (default: mnet_v2, the library default). Larger backbones (r18/r34) detect small/distant faces better at a speed cost; weights auto-download on first use.'),
    ...
    FlagSpec(flag='--face-eye-origin', dest='face_eye_origin', schema_path='gaze.face_eye_origin', kind='store_true', help='Anchor gaze rays at the detected eye midpoint (RetinaFace landmarks) instead of the face-bbox centre. Changes ray origins, so hit counts move vs. blessed baselines (default: off).'),
    ...
    FlagSpec(flag='--rf-onset-samples', dest='rf_onset_samples', schema_path='rayforming.rf_onset_samples', metavar='N', help='Let a NEW face reach fixation eligibility after N pitch/yaw samples instead of the default 5 (half the history buffer). Minimum effective value 2; 0 keeps the default warmup (default: 0). Cuts first-anchor latency for freshly appeared faces.', group='Ray Forming (Gazelle blend)'),
    FlagSpec(flag='--rf-onset-gap', dest='rf_onset_gap', schema_path='rayforming.rf_onset_gap', metavar='N', help='When a never-anchored face wants inference, relax the global call gap to min(--min-call-gap, N) so a new participant is not stuck behind another face\'s recent call. 0 disables (default: 0).', group='Ray Forming (Gazelle blend)'),
