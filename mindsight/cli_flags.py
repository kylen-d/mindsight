...
CORE_FLAGS: tuple[FlagSpec, ...] = (
    ...
    FlagSpec(flag='--save-detections', dest='save_detections', schema_path=None, default=False, kind='store_true', help='Write a per-frame detections side stream ([stem]_detections.csv next to the summary CSV: one row per detection with class, confidence, and bbox). Off by default; the validation suite enables it for its runs to score object IoU.'),
    ...
    FlagSpec(flag='--overlay-theme', dest='overlay_theme', schema_path=None, default='classic', choices=('classic', 'mindsight'), help="Annotated-frame overlay styling: 'classic' is the historical high-saturation look, 'mindsight' uses the brand palette (indigo ink label tabs, logo magenta/jade accents). Cosmetic only -- analysis outputs are unaffected (default: classic)."),
    ...
    FlagSpec(flag='--mgaze-reuse-eps', dest='mgaze_reuse_eps', schema_path='tracker.mgaze_reuse_eps', metavar='EPS', help='Reuse the previous gaze estimate when a face crop is visually unchanged (mean-abs 32x32 grayscale diff below EPS). 0 disables (default: 0.0). Saves per-face model calls on static footage.', group='Performance'),
    ...
    FlagSpec(flag='--model', dest='model', schema_path=None, default='yolo11n.pt', help='Object Detection Model (default: yolo11n.pt since v1.3; yolo11n.onnx in the Models tab is a faster option for CPU-bound installs)'),
    FlagSpec(flag='--face-conf', dest='face_conf', schema_path=None, default=0.5, type='float', metavar='T', help='RetinaFace face-detection confidence threshold (default: 0.5, the library default). Lower to catch small/distant faces.'),
    FlagSpec(flag='--face-input-size', dest='face_input_size', schema_path=None, default=640, type='int', metavar='PX', help='RetinaFace square input resolution in pixels (default: 640, the library default). Raise for small/distant faces at a speed cost.'),
    FlagSpec(flag='--face-model', dest='face_model', schema_path=None, default='r34', choices=('mnet025', 'mnet050', 'mnet_v1', 'mnet_v2', 'r18', 'r34'), metavar='NAME', help='RetinaFace backbone variant (default: r34 -- the v1.3 eval pick: best accuracy AND ~40%% faster than mnet_v2 on the CoreML path; weights auto-download on first use).'),
    ...
    FlagSpec(flag='--face-reid-sim', dest='face_reid_sim', schema_path='tracker.face_reid_sim', metavar='SIM', help='Verify lost-track revivals with ArcFace embeddings: a dead track is re-identified by cosine similarity >= SIM (anywhere in frame, not just near its last position), falling back to positional matching. 0 disables (default: 0.0). Try 0.35-0.45. Embedding weights auto-download on first use (InsightFace research-provenance -- see THIRD_PARTY_LICENSES).'),
    FlagSpec(flag='--face-eye-origin', dest='face_eye_origin', schema_path='gaze.face_eye_origin', kind='store_true', help='Anchor gaze rays at the detected eye midpoint (RetinaFace landmarks) instead of the face-bbox centre (default: ON since v1.3; disable with --no-face-eye-origin).'),
    FlagSpec(flag='--no-face-eye-origin', dest='no_face_eye_origin', schema_path=None, default=False, kind='store_true', help='Anchor gaze rays at the face-bbox centre (the pre-v1.3 behavior), overriding the eye-midpoint default.'),
    ...
    FlagSpec(flag='--rf-gazelle-model', dest='rf_gazelle_model', schema_path=None, default=None, metavar='PATH', help='Path to a gaze-target model for Gazelle blend ray forming: a Gaze-LLE checkpoint (.pt, torch) or a gazelle-dinov3 ONNX export (.onnx, onnxruntime). Used alongside a pitch/yaw backend (MGaze, etc.) to periodically correct rays with scene-aware heatmaps.', group='Ray Forming (Gazelle blend)'),
    ...
    FlagSpec(flag='--rf-inout-gate', dest='rf_inout_gate', schema_path='rayforming.rf_inout_gate', metavar='T', help="Activate the Gaze-LLE checkpoint's in/out-of-frame head (when present) and gate the blend with it: heatmap accepts with an in-frame score below T are vetoed and blend trust is scaled by the score. 0 disables (default: 0.0).", group='Ray Forming (Gazelle blend)'),
    FlagSpec(flag='--rf-reuse-eps', dest='rf_reuse_eps', schema_path='rayforming.rf_reuse_eps', metavar='EPS', help='Skip a scheduled Gaze-LLE call when the scene is visually unchanged since the last real call (mean-abs 64x64 grayscale frame diff below EPS and stable face boxes); the cached heatmaps are re-anchored instead. 0 disables (default: 0.0). Saves forward passes on static footage.', group='Ray Forming (Gazelle blend)'),
    FlagSpec(flag='--rf-onset-samples', dest='rf_onset_samples', schema_path='rayforming.rf_onset_samples', metavar='N', help='Let a NEW face reach fixation eligibility after N pitch/yaw samples instead of the full warmup of 5 (half the history buffer). Minimum effective value 2; 0 keeps the full warmup (default: 3). Cuts first-anchor latency for freshly appeared faces.', group='Ray Forming (Gazelle blend)'),
    FlagSpec(flag='--rf-onset-gap', dest='rf_onset_gap', schema_path='rayforming.rf_onset_gap', metavar='N', help='When a never-anchored face wants inference, relax the global call gap to min(--min-call-gap, N) so a new participant is not stuck behind another face\'s recent call. 0 disables (default: 5).', group='Ray Forming (Gazelle blend)'),
    FlagSpec(flag='--rf-len-refresh-gap', dest='rf_len_refresh_gap', schema_path='rayforming.rf_len_refresh_gap', metavar='N', help='Every N frames, run a cheap half-precision Gaze-LLE pass that refreshes ray LENGTH only -- direction stays with the full-precision fixation-gated corrections. Keeps ray reach fresh between corrections at a fraction of the cost. 0 disables (default: 10).', group='Ray Forming (Gazelle blend)'),
    FlagSpec(flag='--rf-len-slew', dest='rf_len_slew', schema_path='rayforming.rf_len_slew', metavar='N', help='When a length refresh re-latches ray length, ramp the effective length target from the reach currently shown to the new value over N frames instead of snapping, with the length-hold decay paused during the ramp (monotone approach, no bounce). 0 = instant snap (default: 5).', group='Ray Forming (Gazelle blend)'),
    FlagSpec(flag='--rf-len-gain', dest='rf_len_gain', schema_path='rayforming.rf_len_gain', metavar='F', help='Scale the blend ray-length target by F. The v1.3 eval decomposition measured a systematic under-reach (84%% of rays short); 1.10 recovered ~5 px mean / ~8 px median on the eval labels. Applied before smoothing so snap and hit detection see the corrected reach. 1.0 = off (default: 1.10).', group='Ray Forming (Gazelle blend)'),
    FlagSpec(flag='--rf-endpoint-extract', dest='rf_endpoint_extract', schema_path='rayforming.rf_endpoint_extract', choices=('centroid', 'topp'), help="How the Gaze-LLE heatmap becomes a ray-length endpoint: 'centroid' = full-map mass centroid (historical), 'topp' = mass centroid of the top-50%%-mass cells -- diffuse or multi-modal heatmaps no longer drag the endpoint toward the origin (default: centroid).", group='Ray Forming (Gazelle blend)'),
    FlagSpec(flag='--rf-gazelle-fp16', dest='rf_gazelle_fp16', schema_path=None, default=False, kind='store_true', help='Run the blend-path Gaze-LLE model in half precision (CUDA/MPS only; ignored on CPU). Faster per call; output is NOT byte-identical to fp32, so keep it off for regression baselines.', group='Ray Forming (Gazelle blend)'),
    FlagSpec(flag='--rf-gazelle-compile', dest='rf_gazelle_compile', schema_path=None, default=False, kind='store_true', help='torch.compile the blend-path Gaze-LLE model (PyTorch 2+, experimental -- MPS support immature). First call pays a warmup cost.', group='Ray Forming (Gazelle blend)'),
