"""
Plugins/GazeTracking/MGaze/Gaze_Pipeline.py — Per-face gaze estimation pipeline for MGaze.

Handles face cropping, per-face estimation, temporal smoothing, ray
construction, and adaptive-snap logic for the MGaze (gaze-estimation) backend.
Called by MGazePlugin.run_pipeline(), which is invoked by the coordinator
...
def run_mgaze_pipeline(*, frame, faces, gaze_eng, objects, gaze_cfg,
                       smoother=None, snap_hysteresis=None, **kwargs):
    """
    Per-face gaze estimation pipeline step for the MGaze backend.

    Parameters
    ----------
    frame           : BGR numpy array at display resolution.
    faces           : List of face detection dicts (from RetinaFace).
    gaze_eng        : MGaze estimation engine with estimate(crop) method.
    ...
    **kwargs        : Additional context (ignored — future-proofing).
    ...
    adaptive_snap_m = gaze_cfg.adaptive_snap_mode
    ...
    for fi_loc, (entry, (pitch, yaw), fw, gc, bbox) in enumerate(zip(
        ...
        if adaptive_ray and adaptive_targets:
            raw_ctr, raw_snap, _ = adaptive_snap(c, d, adaptive_targets, fb, snap_dist)
            if snap_hysteresis is not None:
                obj_ctr, _ = snap_hysteresis.update(face_track_ids[fi_loc], raw_ctr, raw_snap)
                if obj_ctr is None:
                    end = fb
                elif adaptive_snap_m:
